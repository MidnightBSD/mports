...
#			  default: -c -O1 --prefix=${PREFIX} --single-version-externally-managed --root=${FAKE_DESTDIR}
#
# PYDISTUTILS_EGGINFO
...
_PYTHON_BASECMD=		${LOCALBASE:S;^${FAKE_DESTDIR};;}/bin/python
...
CRYPTOGRAPHY_DEPENDS=	${PYTHON_PKGNAMEPREFIX}cryptography>=0:security/py-cryptography@${PY_FLAVOR}
.  else
CRYPTOGRAPHY_DEPENDS=	${PYTHON_PKGNAMEPREFIX}cryptography-legacy>=0:security/py-cryptography-legacy@${PY_FLAVOR}
.  endif
...
CYTHON0_DEPENDS=${PYTHON_PKGNAMEPREFIX}cython0>=0.29.37:lang/cython0@${PY_FLAVOR}
...
_UNIQUE_FIND_SUFFIX_FILES=	${SED} -e 's|^${TRUE_PREFIX}/||' ${_PYTHONPKGLIST} ${TMPPLIST}
.else
_UNIQUE_FIND_SUFFIX_FILES=	${SED} -e 's|^${TRUE_PREFIX}/||' ${TMPPLIST} 2>/dev/null
.endif
...
PY_TOMLI=		${PYTHON_PKGNAMEPREFIX}tomli>=2.3:textproc/py-tomli@${PY_FLAVOR}
