...
kodi-autoplist:
	@${FIND} -ds ${FAKE_DESTDIR}${PREFIX}/lib/kodi/addons/${KODI_ADDON} \( -type f -or -type l \) -print | ${SED} -E -e \
		's,^${FAKE_DESTDIR}${PREFIX}/?,,' >> ${TMPPLIST}

	@if [ -d ${FAKE_DESTDIR}${PREFIX}/share/kodi/addons/${KODI_ADDON} ]; then \
		${FIND} -ds ${FAKE_DESTDIR}${PREFIX}/share/kodi/addons/${KODI_ADDON} -type f -print | ${SED} -E -e \
		's,^${FAKE_DESTDIR}${PREFIX}/?,,' >> ${TMPPLIST} ; \
	fi
.  endif
