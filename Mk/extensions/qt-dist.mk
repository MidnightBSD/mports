...
# LAH: TODO why is this not getting the qtver?
_QT_VER=	5
.include "${MPORTEXTENSIONS}/qt.mk"
qmake_ARGS?=	# empty
.include "${MPORTEXTENSIONS}/qmake.mk"
...
_QT5_MASTER_SITES_kde=		FREEBSD_LOCAL
...
# devel/qt6-openapi needs an offline archive of the maven deps, which is not
# published by upstream. Fetch the one FreeBSD generates and mirrors, the same
# way the Qt5 KDE distfiles are sourced.
.  if ${_QT_DIST} == openapi && !defined(QTOPENAPI_MAINTAINER_MODE)
_QT6_MASTER_SITES+=		FREEBSD_LOCAL/kde/KDE/Qt/${_QT_VERSION}:maven
.  endif
...
# install and deinstall step.
	@${SED} -e 's,%%QT_MODNAME%%,${QT_MODNAME},g' \
		-e 's,%%QT_INCDIR%%,${LOCALBASE}/${QT_INCDIR_REL},g' \
...
.      endfor
	@${ECHO_CMD} "${QT_INCDIR_REL}/QtCore/modules/qconfig-${QT_MODNAME}.h" \
		>> ${TMPPLIST}
	@${ECHO_CMD} "@exec echo '#include <QtCore/modules/qconfig-${QT_MODNAME}.h>' >> ${TRUE_PREFIX}/${QT_INCDIR_REL}/QtCore/qconfig-modules.h" \
		>> ${TMPPLIST}
.    endif # ${QT_DEFINES:N-*}
.    if ${QT_CONFIG:N-*}
