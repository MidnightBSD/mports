...
ELIXIR_APP_ROOT?=	${TRUE_PREFIX}/lib/elixir/lib/${ELIXIR_APP_NAME}
...
do-install:
	@${MKDIR} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}
	@${MKDIR} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/lib
	cd ${WRKSRC}/lib && ${COPYTREE_SHARE} \* ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/lib
	@${MKDIR} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/ebin
	${INSTALL_DATA} ${WRKSRC}/_build/${MIX_BUILD_NAME}/lib/${ELIXIR_APP_NAME}/ebin/* \
		${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/ebin
	if [ -d ${WRKSRC}/priv ]; then \
		${MKDIR} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/priv; \
		cd ${WRKSRC}/priv && ${COPYTREE_SHARE} \* ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/priv; \
	fi
...
.    for dir in ${MIX_DOC_DIRS}
	cd ${WRKSRC} && ${COPYTREE_SHARE} ${dir} ${FAKE_DESTDIR}${DOCSDIR}
.    endfor
.    for file in ${MIX_EXTRA_FILES}
	${INSTALL_DATA} ${WRKSRC}/${file} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}
.    endfor
.    for dir in ${MIX_EXTRA_DIRS}
	@${MKDIR} ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/${dir}
	cd ${WRKSRC}/${dir} && ${COPYTREE_SHARE} . ${FAKE_DESTDIR}${ELIXIR_APP_ROOT}/${dir}
.    endfor
.    for app in ${MIX_EXTRA_APPS}
