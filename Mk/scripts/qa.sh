...
list_stagedir_elfs() {
	cd ${FAKE_DESTDIR} && find -s . -type f \( -perm +111 -o -name '*.so*' \) "$@"
}
...
shebangonefile() {
	...
	case "${f}" in
	*.pm|*.pod|*.txt|${FAKE_DESTDIR}${LINUXBASE}/*)
		return 0
		;;
	esac
	...
	*)
	...
	if [ -n "${badinterp}" ]; then
		err "'${badinterp}' is an invalid shebang you need USES=shebangfix for '${f#${FAKE_DESTDIR}${PREFIX}/}'"
		rc=1
	fi
	...
	while read -r l; do
		...
		case "${link}" in
			...
			/*)
				...
				if [ -e "${FAKE_DESTDIR}${link}" ]; then
					warn "Bad symlink '${l#${FAKE_DESTDIR}}' pointing to an absolute pathname '${link}'"
				fi
				# Also warn if the symlink exists nowhere.
				if [ ! -e "${FAKE_DESTDIR}${link}" -a ! -e "${link}" ]; then
					warn "Symlink '${l#${FAKE_DESTDIR}}' pointing to '${link}' which does not exist in the stage directory or in localbase"
				fi
				;;
		esac
	# Use heredoc to avoid losing rc from find|while subshell.
	done <<-EOF
	$(find ${FAKE_DESTDIR} -type l -exec stat -f "%N${LF}%Y" {} +)
	EOF
...
paths() {
	...
	while read -r f; do
		...
		err "'${f#${FAKE_DESTDIR}${PREFIX}/}' is referring to ${FAKE_DESTDIR}"
		rc=1
	# Use heredoc to avoid losing rc from find|while subshell
	done <<-EOF
	$(find ${TMPPLIST} ${FAKE_DESTDIR} -type f -exec grep -l "${FAKE_DESTDIR}" {} +)
	EOF
...
stripped() {
	...
	find ${FAKE_DESTDIR} -type f ! -name '*.a' ! -name '*.o' \
	    ...
	    while read -r f; do
		warn "'${f#${FAKE_DESTDIR}${PREFIX}/}' is not stripped consider trying INSTALL_TARGET=install-strip or using \${STRIP_CMD}"
	done
}

desktopfileutils() {
	if [ -z "${USESDESKTOPFILEUTILS}" ]; then
		grep -q MimeType= ${FAKE_DESTDIR}${PREFIX}/share/applications/*.desktop 2>/dev/null &&
...
sharedmimeinfo() {
	...
	found=0
	for f in ${FAKE_DESTDIR}${PREFIX}/share/mime/packages/*.xml; do
		[ "${f}" = "${FAKE_DESTDIR}${PREFIX}/share/mime/packages/*.xml" ] && break #no matches
		[ "${f}" = "${FAKE_DESTDIR}${PREFIX}/share/mime/packages/freedesktop.org.xml" ] && continue
		found=1
		break
	done
...
suidfiles() {
	...
	filelist=$(find ${FAKE_DESTDIR} -type f \
...
libtool() {
	if [ -z "${USESLIBTOOL}" ]; then
		find ${FAKE_DESTDIR} -name '*.la' | while read -r f; do
...
libperl() {
	...
	if [ -n "${SITE_ARCH_REL}" -a -d "${FAKE_DESTDIR}${PREFIX}/${SITE_ARCH_REL}" ]; then
		...
		$(find ${FAKE_DESTDIR}${PREFIX}/${SITE_ARCH_REL} -name '*.so')
		EOT
...
prefixvar() {
	if [ ${PREFIX} != ${LINUXBASE} -a -d ${FAKE_DESTDIR}${PREFIX}/var ]; then
...
terminfo() {
	...
	for f in ${FAKE_DESTDIR}${PREFIX}/share/misc/*.terminfo; do
		[ "${f}" = "${FAKE_DESTDIR}${PREFIX}/share/misc/*.terminfo" ] && break #no matches
		found=1
		break
	done
	for f in ${FAKE_DESTDIR}${PREFIX}/share/misc/terminfo.db*; do
		[ "${f}" = "${FAKE_DESTDIR}${PREFIX}/share/misc/terminfo.db*" ] && break #no matches
		found=1
		break
	done
...
proxydeps() {
	...
	while read -r file; do
		...
		while read -r dep_file; do
			...
			if mport which -q ${dep_file} > /dev/null 2>&1; then
				dep_file_pkg=$(mport which -qo ${dep_file})
...
sonames() {
	[ ! -d ${FAKE_DESTDIR}${PREFIX}/lib -o -n "${BUNDLE_LIBS}" ] && return 0
	while read -r f; do
		# No results presents a blank line from heredoc.
		[ -z "${f}" ] && continue
		# Ignore symlinks
	...
	$(find ${FAKE_DESTDIR}${PREFIX}/lib -name '*.so.*')
	EOT
}
	...
	for message in ${PKGMESSAGES}; do
		if [ -f "${message}" ]; then
			if ! head -1 "${message}" | grep -q '^\['; then
				...
				warn "See https://github.com/MidnightBSD/src/wiki/mports-porting-guide"
			fi
		fi
	done
