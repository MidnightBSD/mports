...
#
#   Combined size of mports: 510.5 Mb
#   The Top 20 mports by size account for 15.14% of the collection
#   ====================================================================
#    [snipped 15 rows]
...
cd /usr/mports && ${DUK} | awk "${AWKCMD1}" | sort -n | tail -n ${TOP} \
	> ${SCRATCH}

total=$(du -sk /usr/mports/[a-z]* | awk "${AWKCMD2}")
...
printf "Combined size of mports: %1.1f Mb\n" ${megabytes}
printf "The Top %d mports by size account for %1.2f%% of the collection\n" \
	${TOP} ${PC}
echo ${DASH}
