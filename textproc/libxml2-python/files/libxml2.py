...
def htmlCreateFileParserCtxt(filename, encoding):
    """DEPRECATED: Use htmlNewParserCtxt and htmlCtxtReadFile. A
...
def htmlNewParserCtxt():
    """This can be used to parse HTML documents into DOM trees
...
def htmlParseDoc(cur, encoding):
    """DEPRECATED: Use htmlReadDoc. This function uses deprecated
...
def htmlReadFd(fd, URL, encoding, options):
    """NOTE that the file descriptor will not be closed when the
       context is freed or reset. See htmlCtxtReadFd for details. """
    ret = libxml2mod.htmlReadFd(fd, URL, encoding, options)
    if ret is None:raise treeError('htmlReadFd() failed')
    return xmlDoc(_obj=ret)

def htmlReadFile(URL, encoding, options):
    """See htmlCtxtReadFile for details. """
    ret = libxml2mod.htmlReadFile(URL, encoding, options)
...
def htmlIsBooleanAttr(name):
    """This doesn't handle HTML5. DEPRECATED: Internal function,
       don't use. """
...
def catalogAdd(type, orig, replace):
    """If called before any other catalog routine, allows to
...
def loadCatalog(filename):
    """It will recurse in SGML CATALOG entries. this function is
...
def loadCatalogs(paths):
    """this function is not thread safe, catalog initialization
       should preferably be done once at startup """
    libxml2mod.xmlLoadCatalogs(paths)

def loadSGMLSuperCatalog(filename):
    """It won't expand CATALOG or DELEGATE references. This is
...
def parseCatalogFile(filename):
    """It's like xmlParseFile except it bypass all catalog
       lookups. DEPRECATED: Internal function, don't use """
...
def addEncodingAlias(name, alias):
    """Existing aliases will be overwritten. DEPRECATED: This
      function modifies global state and is not thread-safe. See
       xmlCtxtSetCharEncConvImpl for an alternative. """
...
def initParser():
    """For older versions, it's recommended to call this function
...
def parseDTD(publicId, systemId):
    ret = libxml2mod.xmlParseDTD(publicId, systemId)
    if ret is None:raise parserError('xmlParseDTD() failed')
    return xmlDtd(_obj=ret)

def parseDoc(cur):
    """This function uses deprecated global variables to set
...
def readFd(fd, URL, encoding, options):
    """See xmlCtxtReadFd for details. NOTE that the file
...
def readFile(URL, encoding, options):
    """If a "-" filename is passed, the function will read from
      stdin. This feature is potentially insecure and might be
      removed from later versions. See xmlCtxtReadFile for
       details. """
    ret = libxml2mod.xmlReadFile(URL, encoding, options)
...
def recoverMemory(buffer, size):
    """In the case the document is not Well Formed, an attempt to
      ...
      global variables to set parser options which default to
      XML_PARSE_NODICT | XML_PARSE_RECOVER. DEPRECATED: Use
       xmlReadMemory with XML_PARSE_RECOVER. """
...
def thrDefKeepBlanksDefaultValue(v):
    """DEPRECATED: Whitespace is kept by default. Use
       xmlParserOption XML_PARSE_NOBLANKS to remove whitespace. """
...
def checkLanguageID(lang):
    """DEPRECATED: Internal function, do not use. NOTE: this is
...
def isLetter(c):
    """DEPRECATED: Internal function, don't use. [84] Letter ::=
...
def buildQName(ncname, prefix, memory, len):
    """Builds the QName prefix:ncname in memory if there is enough
      space and prefix is not None nor empty, otherwise allocate
       a new string. If prefix is None or empty it returns ncname. """
...
def newDoc(version):
    """If version is None, "1.0" is used. """
...
def setCompressMode(mode):
    """DEPRECATED: Use xmlSetDocCompressMode Correct values: 0
...
def URIEscape(str):
    """25 May 2001 Uses xmlParseURI and xmlURIEscapeStr to try to
       escape correctly according to RFC2396. Carl Douglas """
...
def URIUnescapeString(str, len, target):
    """The output is a direct unsigned char translation of XX
      values (no encoding) Note that the length of the result can
       only be smaller or same size as the input string. """
...
def canonicPath(path):
    """If the path contains the substring "://", it is considered
...
def thrDefIndentTreeOutput(v):
    """DEPRECATED: Indenting is enabled by default. Use the
...
def thrDefSaveNoEmptyTags(v):
    """DEPRECATED: Use the xmlsave.h API and xmlSaveOption
       XML_SAVE_NO_EMPTY. """
...
def checkUTF8(utf):
    """utf is assumed to be null-terminated. This function is not
...
class xmlNode(xmlCore):
    ...
    def addChild(self, cur):
        """If cur is a text node, it may be merged with an adjacent
    ...
    def copyNode(self, recursive):
        """If extended is 0, make a shallow copy. If extended is 1,
          ...
          including properties and namespaces of elements. Use of
           this function is DISCOURAGED in favor of xmlDocCopyNode. """
    ...
    def copyPropList(self, cur):
        """This function sets the parent pointers of the copied
    ...
    def docCopyNode(self, doc, recursive):
        """If extended is 0, make a shallow copy. If extended is 1,
          make a deep copy (properties, namespaces and children when
          applicable). If extended is 2, make a shallow copy
           including properties and namespaces of elements. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def docSetRootElement(self, doc):
        """To remove a node from a document, use xmlUnlinkNode
    ...
    def freeNode(self):
        """This doesn't unlink the node from the tree. Call
           xmlUnlinkNode first unless cur is a root node. """
    ...
    def getBase(self, doc):
        """This function doesn't allow to distinguish memory
           allocation failures from a non-existing base. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def getContent(self):
        """For text nodes, the string value is the text content.
    ...
    def getLang(self):
        """Look up the value of the xml:lang attribute or the one
           carried by the nearest ancestor. """
        ret = libxml2mod.xmlNodeGetLang(self._o)
        return ret

    def getSpacePreserve(self):
        """Look up the value of the xml:space attribute or the one
           carried by the nearest ancestor. """
        ret = libxml2mod.xmlNodeGetSpacePreserve(self._o)
        return ret

    def hasNsProp(self, name, nameSpace):
        """The attribute has to match the specified namespace. A
    ...
    def hasProp(self, name):
        """This function also looks in DTD attribute declaration for
           #FIXED or default declaration values. """
    ...
    def lineNo(self):
        """Try to work around the limitation of line numbers being
    ...
    def listGetRawString(self, doc, inLine):
        """If inLine is true, entity references will be substituted.
          ...
          characters like & will be escaped. See
           xmlEncodeSpecialChars for details. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def newChild(self, ns, name, content):
        """If ns is None, the newly created element inherits the
          ...
          character and entity references. Text and entity reference
           node will be added to the child element, see xmlNewDocNode. """
    ...
    def newNs(self, href, prefix):
        """For a default namespace, prefix should be None. The
          ...
          namespace declarations. It is an error if the node already
           has a definition for the prefix or default namespace. """
    ...
    def newNsProp(self, ns, name, value):
        """If provided, value should be a raw, unescaped string. If
    ...
    def noNsProp(self, name):
        """Entities are substituted. The returned value must be freed
          ...
          doesn't allow to distinguish malloc failures from missing
           attributes. It's more robust to use xmlNodeGetAttrValue. """
    ...
    def reconciliateNs(self, doc):
        """This is needed for example after copy or cut and then paste
          ...
          If not possible the new namespaces are redeclared on tree
           at the top of the given subtree. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def replaceNode(self, cur):
        """If cur is provided, it is unlinked and inserted in place of
          old . It is an error if old has no parent. Unlike
          xmlAddChild, this function doesn't merge text nodes or
           delete duplicate attributes. See the notes in xmlAddChild. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
    ...
    def setContent(self, content):
        """Sets the raw text content of text, CDATA, comment or PI
    ...
    def setNs(self, ns):
        """Passing a None namespace unsets the namespace. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def setProp(self, name, value):
        """If name has a prefix, the corresponding namespace will be
    ...
    def textMerge(self, second):
        """If first is None, second is returned. Otherwise, the second
    ...
    def unlinkNode(self):
        """The node is not freed. Unless it is reinserted, it must be
    ...
    def isID(self, doc, attr):
        """In case we have DTD(s) then this is done if DTD loading has
          been requested. In case of HTML documents parsed with the
           HTML parser, ID detection is done systematically. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def isRef(self, doc, attr):
        """In case we have DTD(s) then this is simple, otherwise we
    ...
    def validNormalizeAttributeValue(self, doc, name, value):
        """DEPRECATED: Internal function, don't use. If the declared
    ...
    def xpathNodeEval(self, str, ctx):
        """The node 'node' is set as the context node. The context
           node is not restored. """
...
class xmlDoc(xmlNode):
    ...
    def htmlAutoCloseTag(self, name, elem):
        ...
        ret = libxml2mod.htmlAutoCloseTag(self._o, name, elem__o)
        return ret
    ...
    def htmlIsAutoClosed(self, elem):
        ...
        ret = libxml2mod.htmlIsAutoClosed(self._o, elem__o)
        return ret
    ...
    def htmlDocDump(self, f):
        """Uses the encoding of the document. If the document has no
          ...
          unconditionally which is typically undesired. Use of this
           function is DISCOURAGED in favor of htmlNodeDumpFileFormat. """
        ret = libxml2mod.htmlDocDump(f, self._o)
        return ret

    def htmlGetMetaEncoding(self):
        """The returned string points into attribute content and can
          contain trailing garbage. It should be copied before
           modifying or freeing nodes. """
    ...
    def htmlNodeDumpFileFormat(self, out, cur, encoding, format):
        """If encoding is None, ASCII with HTML 4.0 named character
          entities will be used. This is inefficient compared to
           UTF-8 and might be changed in a future version. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
    ...
    def htmlSaveFile(self, filename):
        """Same as htmlSaveFileFormat with encoding set to None and
    ...
    def htmlSaveFileFormat(self, filename, encoding, format):
        """If filename is "-" , stdout is used. This is potentially
          ...
          UTF-8 and might be changed in a future version. Sets or
           updates meta tags containing the character encoding. """
    ...
    def addDocEntity(self, name, type, publicId, systemId, content):
        """xmlAddEntity offers better error handling. """
        ret = libxml2mod.xmlAddDocEntity(self._o, name, type, publicId, systemId, content)
        if ret is None:raise treeError('xmlAddDocEntity() failed')
        __tmp = xmlEntity(_obj=ret)
        return __tmp

    def addDtdEntity(self, name, type, publicId, systemId, content):
        """xmlAddEntity offers better error handling. """
        ret = libxml2mod.xmlAddDtdEntity(self._o, name, type, publicId, systemId, content)
    ...
    def encodeEntitiesReentrant(self, input):
        """If doc is None or an XML document, replaces < , > and &
          ...
          characters like ASCII control codes. See
           xmlEncodeSpecialChars for an alternative. """
    ...
    def newEntity(self, name, type, publicId, systemId, content):
        """Like xmlAddDocEntity, but if doc is None or has no internal
          subset defined, an unlinked entity will be returned. It is
          then the responsibility of the caller to link it to the
           document later or free it when not needed anymore. """
        ret = libxml2mod.xmlNewEntity(self._o, name, type, publicId, systemId, content)
    ...
    def ctxtValidateDocument(self, ctxt):
        """Like xmlValidateDocument but uses the parser context's
          error handler. Option XML_PARSE_DTDLOAD should be enabled
          in the parser context to make external entities work. 2.14.0 """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
    ...
    def ctxtValidateDtd(self, ctxt, dtd):
        """Like xmlValidateDtd but uses the parser context's error
           handler. 2.14.0 """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
    ...
    def copyDoc(self, recursive):
        """If recursive, the content tree will be copied too as well
           as DTD, namespaces and entities. """
        ret = libxml2mod.xmlCopyDoc(self._o, recursive)
        if ret is None:raise treeError('xmlCopyDoc() failed')
        __tmp = xmlDoc(_obj=ret)
        return __tmp

    def copyNode(self, node, recursive):
        """If extended is 0, make a shallow copy. If extended is 1,
          make a deep copy (properties, namespaces and children when
          applicable). If extended is 2, make a shallow copy
           including properties and namespaces of elements. """
        ...
        ret = libxml2mod.xmlDocCopyNode(node__o, self._o, recursive)
        if ret is None:raise treeError('xmlDocCopyNode() failed')
        __tmp = xmlNode(_obj=ret)
        return __tmp
    ...
    def createIntSubset(self, name, publicId, systemId):
        """If a document is provided and it already has an internal
          subset, the existing DTD object is returned without
          creating a new object. If the document has no internal
           subset, it will be set to the created DTD. """
        ret = libxml2mod.xmlCreateIntSubset(self._o, name, publicId, systemId)
    ...
    def dump(self, f):
        """Uses the document's encoding. If the document has no
           encoding, uses ASCII without an encoding declaration. """
    ...
    def getRootElement(self):
        """Searches the document's children for the root element. The
          first child is not necessarily the root element, but could
           also be a DTD, comment or PI. """
    ...
    def newDocNode(self, ns, name, content):
        """If provided, content is expected to be a valid XML
          ...
          document only owns nodes reachable from the root node.
           Unlinked subtrees must be freed manually. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def newDocNodeEatName(self, ns, name, content):
        """Like xmlNewDocNode, but the name string will be used
          directly without making a copy. Takes ownership of name
           which will also be freed on error. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def newDocProp(self, name, value):
        """If provided, value is expected to be a valid XML attribute
    ...
    def newDtd(self, name, publicId, systemId):
        """If a document is provided, it is an error if it already has
          an external subset. If the document has no external subset,
          it will be set to the created DTD. To create an internal
           subset, use xmlCreateIntSubset. """
        ret = libxml2mod.xmlNewDtd(self._o, name, publicId, systemId)
    ...
    def stringGetNodeList(self, value):
        """The resulting nodes will be associated with the document if
          ...
          validated. Syntax errors or references to undeclared
           entities will be ignored silently with unspecified results. """
    ...
    def validCtxtNormalizeAttributeValue(self, ctxt, elem, name, value):
        """DEPRECATED: Internal function, don't use. If the declared
          ...
          space (#x20) character. Also check VC: Standalone Document
           Declaration in P32, and update ctxt->valid accordingly """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if elem is None: elem__o = None
        else: elem__o = elem._o
    ...
    def validateDocument(self, ctxt):
        """DEPRECATED: This function can't report malloc or other
          failures. Use xmlCtxtValidateDocument. Performs the all the
          checks described by the XML Rec, i.e. validates the
    ...
    def validateDocumentFinal(self, ctxt):
        """DEPRECATED: Internal function, don't use. Performs the
    ...
    def validateNotationUse(self, ctxt, notationName):
        """[ VC: Notation Declared ] DEPRECATED: Internal function,
           don't use. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
    ...
    def validateRoot(self, ctxt):
        """Performs the following check as described by the XML-1.0
          recommendation: DEPRECATED: Internal function, don't use. [
          VC: Root Element Type ] It doesn't try to recurse or apply
           other checks to the element. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
    ...
    def xpathOrderDocElems(self):
        """Evaluation functions. This stamps all the element nodes
          ...
          stored is actually - the node number (starting at -1) to be
           able to differentiate from line numbers. """
...
class parserCtxt(parserCtxtCore):
    ...
    def htmlCtxtReadFd(self, fd, URL, encoding, options):
        """See htmlCtxtUseOptions for details. NOTE that the file
          descriptor will not be closed when the context is freed or
           reset. """
    ...
    def htmlCtxtReadMemory(self, buffer, size, URL, encoding, options):
        """The input buffer must not contain any terminating null
           bytes. See htmlCtxtUseOptions for details. """
    ...
    def htmlCtxtSetOptions(self, options):
        """Unset options are cleared. 2.14.0 With older versions, you
           can use htmlCtxtUseOptions. """
        ret = libxml2mod.htmlCtxtSetOptions(self._o, options)
        return ret

    def htmlCtxtUseOptions(self, options):
        """The following options are never cleared and can only be
    ...
    def htmlParseDocument(self):
        """This is useful if you're only interested in custom SAX
    ...
    def byteConsumed(self):
        """This function is computed in bytes from the beginning
    ...
    def ctxtGetNode(self):
        """This is only useful if the default SAX callbacks which
          ...
          element node, or the document node if no element was parsed
           yet. 2.15.0 """
        ret = libxml2mod.xmlCtxtGetNode(self._o)
        if ret is None:raise treeError('xmlCtxtGetNode() failed')
        __tmp = xmlNode(_obj=ret)
        return __tmp
    ...
    def ctxtIsInSubset(self):
        """Should only be used by SAX callbacks. Return values are 0:
          not in DTD 1: in internal DTD subset 2: in external DTD
           subset 2.15.0 """
        ret = libxml2mod.xmlCtxtIsInSubset(self._o)
        return ret

    def ctxtIsStopped(self):
        """The parser is stopped on fatal (non-wellformedness) errors
           or on user request with xmlStopParser . 2.14.0 """
    ...
    def ctxtReadFd(self, fd, URL, encoding, options):
        """NOTE that the file descriptor will not be closed when the
          context is freed or reset. URL is used as base to resolve
           external entities and for error reporting. """
    ...
    def ctxtSetOptions(self, options):
        """Unset options are cleared. 2.13.0 With older versions, you
           can use xmlCtxtUseOptions. """
    ...
    def parseExtParsedEnt(self):
        """DEPRECATED: Internal function, don't use. [78] extParsedEnt
           ::= TextDecl? content """
        ret = libxml2mod.xmlParseExtParsedEnt(self._o)
        return ret

    def setupParserForBuffer(self, buffer, filename):
        """The buffer parameter must not be None, but the filename
           parameter can be DEPRECATED: Don't use. """
    ...
    def parseAttValue(self):
        """DEPRECATED: Internal function, don't use. [10] AttValue ::=
          '"' ([^<&"] | Reference)* '"' |
                  "'"
    ...
    def parseAttributeListDecl(self):
        """Always consumes '<!'. DEPRECATED: Internal function, don't
...
[21] CDEnd ::= ']]>' """
 ...
 must not occur within comments. " [15]
    ...
    def parseDocTypeDecl(self):
        """DEPRECATED: Internal function, don't use. [28] doctypedecl
    ...
    def parseElement(self):
        """DEPRECATED: Internal function, don't use. [39] element ::=
    ...
    def parseEncName(self):
        """DEPRECATED: Internal function, don't use. [81] EncName ::=
           [A-Za-z] ([A-Za-z0-9._] | '-')* """
        ret = libxml2mod.xmlParseEncName(self._o)
        return ret

    def parseEncodingDecl(self):
        """DEPRECATED: Internal function, don't use. [80] EncodingDecl
          ::= S 'encoding' Eq ('"' EncName '"' | 
                   
             "'" EncName "'") this setups the conversion filters. """
        ret = libxml2mod.xmlParseEncodingDecl(self._o)
        return ret

    def parseEndTag(self):
        """DEPRECATED: Internal function, don't use. [42] ETag ::=
...
[76] NDataDecl ::= S 'NDATA' S
    ...
    def parseExternalSubset(self, publicId, systemId):
        """DEPRECATED: Internal function, don't use. [30] extSubset
...
[31] extSubsetDecl ::=
          (markupdecl | conditionalSect |
           ...
           PEReference | S) * """
        libxml2mod.xmlParseExternalSubset(self._o, publicId, systemId)

    def parseMarkupDecl(self):
        """Always consumes '<!' or '<?'. DEPRECATED: Internal
    ...
    def parseMisc(self):
        """DEPRECATED: Internal function, don't use. [27] Misc ::=
           Comment | PI |  S """
        libxml2mod.xmlParseMisc(self._o)

    def parseName(self):
        """DEPRECATED: Internal function, don't use. [4] NameChar ::=
          Letter | Digit | '.' | '-' | '_' | ':' |
...
[6] Names ::= Name (#x20 Name)* """
    ...
    def parseNmtoken(self):
        """DEPRECATED: Internal function, don't use. [7] Nmtoken ::=
           (NameChar)+
...
'SYSTEM' S SystemLiteral See the NOTE on
    ...
    def parsePI(self):
        """DEPRECATED: Internal function, don't use. [16] PI ::= '<?'
    ...
    def parsePITarget(self):
        """DEPRECATED: Internal function, don't use. [17] PITarget ::=
           Name - (('X' | 'x') ('M' | 'm') ('L' | 'l')) """
        ret = libxml2mod.xmlParsePITarget(self._o)
        return ret

    def parsePubidLiteral(self):
        """DEPRECATED: Internal function, don't use. [12] PubidLiteral
           ::= '"' PubidChar* '"' | "'" (PubidChar - "'")* "'" """
    ...
    def parseSDDecl(self):
        """DEPRECATED: Internal function, don't use. [32] SDDecl ::= S
          'standalone' Eq
                (("'" ('yes' | 'no') "'") |
...
[25]
    ...
    def parseVersionNum(self):
        """DEPRECATED: Internal function, don't use. [26] VersionNum
          ::= '1.' [0-9]+ In practice allow [0-9].[0-9]+ at that level """
        ret = libxml2mod.xmlParseVersionNum(self._o)
        return ret

    def parseXMLDecl(self):
        """DEPRECATED: Internal function, don't use. [23] XMLDecl ::=
           '<?xml' VersionInfo EncodingDecl? SDDecl? S? '?>' """
        libxml2mod.xmlParseXMLDecl(self._o)

    def parserHandlePEReference(self):
        """[69] PEReference ::= '%' Name ';' DEPRECATED: Internal
          ...
          i.e. Included in literal in entity values Included as
           Parameter Entity reference within DTDs """
    ...
    def switchEncodingName(self, encoding):
        """This overrides the encoding found in the XML declaration.
          This function can also be used to override the encoding of
           chunks passed to xmlParseChunk . 2.13.0 """
...
class xmlAttr(xmlNode):
    ...
    def removeProp(self):
        """Note this doesn't work for namespace declarations. The
...
class xmlDtd(xmlNode):
    ...
    def dtdElementDesc(self, name):
        """NOTE: A None return value can also mean that a memory
           allocation failed. """
...
class xmlNs(xmlNode):
    ...
    def newNodeEatName(self, name):
        """Use of this function is DISCOURAGED in favor of
          xmlNewDocNodeEatName. Like xmlNewNode, but the name string
          will be used directly without making a copy. Takes
    ...
    def xpathNodeSetFreeNs(self):
        """Really internal functions. In a node set the namespace
...
class xmlReg:
    ...
    def regexpExec(self, value):
        ret = libxml2mod.xmlRegexpExec(self._o, value)
        return ret
    ...
    def regexpIsDeterminist(self):
        """DTD and XML Schemas require a deterministic content model,
          ...
          outgoing edges, causing serious worst-case performance
           issues. DEPRECATED: : Internal function, don't use. """
...
class relaxNgParserCtxt:
    ...
    def relaxParserSetFlag(self, flag):
        ret = libxml2mod.xmlRelaxParserSetFlag(self._o, flag)
        return ret

    def relaxParserSetIncLImit(self, limit):
        """Set to 0 to use the default value. """
        ret = libxml2mod.xmlRelaxParserSetIncLImit(self._o, limit)
        return ret
...
class relaxNgValidCtxt(relaxNgValidCtxtCore):
    ...
    def relaxNGValidCtxtClearErrors(self):
        """it doesn's reset the last internal libxml2 error """
        libxml2mod.xmlRelaxNGValidCtxtClearErrors(self._o)
    ...
    def RelaxNGValidateCtxt(self, reader, options):
        """Activation is only possible before the first Read(). If
          ctxt is None, then RelaxNG schema validation is deactivated. """
        if reader is None: reader__o = None
        else: reader__o = reader._o
...
class SchemaValidCtxt(SchemaValidCtxtCore):
    ...
    def SchemaValidateCtxt(self, reader, options):
        """Activation is only possible before the first Read(). If
           ctxt is None, then XML Schema validation is deactivated. """
        if reader is None: reader__o = None
        else: reader__o = reader._o
...
class xmlTextReader(xmlTextReaderCore):
    ...
    def CurrentDoc(self):
        """NOTE: as a result of this call, the reader will not destroy
          the associated XML document and calling xmlFreeDoc on the
           result is needed once the reader parsing has finished. """
    ...
    def CurrentNode(self):
        """This is dangerous because the underlying node may be
           destroyed on the next Reads. """
    ...
    def Expand(self):
        """It then makes the subtree available until the next
    ...
    def GetRemainder(self):
        """this method stops the parser, set its state to End Of File
    ...
    def Normalization(self):
        """Since attribute value and end of line normalizations are a
    ...
    def Preserve(self):
        """The caller must also use xmlTextReaderCurrentDoc to keep an
    ...
    def RelaxNGValidate(self, rng):
        """Activation is only possible before the first Read(). If rng
           is None, then RelaxNG schema validation is deactivated. """
    ...
    def SchemaValidate(self, xsd):
        """Activation is only possible before the first Read(). If xsd
           is None, then XML Schema validation is deactivated. """
    ...
    def SetParserProp(self, prop, value):
        """Note that some properties can only be changed before any
           read has been done. """
...
class xpathContext:
    ...
    def xpathContextSetCache(self, active, value, options):
        """If activates XPath objects (xmlXPathObject) will be cached
          internally to be reused. options must be set to 0 to enable
          XPath object caching. Other values for options have
    ...
    def xpathRegisterNs(self, prefix, ns_uri):
        """Extending a context. If ns_uri is None it unregisters the
           namespace """
...
class xpathParserContext:
    ...
    def xpathIdFunction(self, nargs):
        """When the argument to id is of type node-set, then the
    ...
    def xpathLangFunction(self, nargs):
        """The language of the context node is determined by the value
    ...
    def xpathNamespaceURIFunction(self, nargs):
        """If the node-set is empty, the first node has no name, or
    ...
    def xpathPositionFunction(self, nargs):
        """The first position is 1, and so the last position will be
           equal to last(). """
    ...
    def xpathRoundFunction(self, nargs):
        """If there are two such numbers, then the one that is closest
           to positive infinity is returned. """
    ...
    def xpathStringFunction(self, nargs):
        """A node-set is converted to a string by returning the value
    ...
    def xpathSubstringFunction(self, nargs):
        """For example, substring("12345",2,3) returns "234". If the
    ...
    def xpathTranslateFunction(self, nargs):
        """For example, translate("bar","abc","ABC") returns the
